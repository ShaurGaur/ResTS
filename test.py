...
model.load_weights('./ResTS-full.h5')
...
def load_dataset(file_txt=TEST_TXT):
    df0 = pd.read_csv(file_txt, sep='\t', header=None, names=['img_path', 'label_num'])
    file_paths = df0['img_path'].tolist()
    labels = df0['label_num'].tolist()
...
def preprocess_and_duplicate_labels(x, y):
    x = xception_preprocess_input(x)  # Apply model-specific preprocessing
